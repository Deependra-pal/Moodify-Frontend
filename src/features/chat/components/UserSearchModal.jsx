      ...
      <div className="bg-[#121214] border border-white/10 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="p-4 border-b border-white/5 flex items-center justify-between">
          <h3 className="text-base font-bold text-white flex items-center gap-2">
            <UserPlus className="h-4.5 w-4.5 text-[#1db954]" />
            Find & Add Friends
          </h3>
          <button
            onClick={onClose}
        ...
        {/* Search Bar */}
        <form onSubmit={handleSearch} className="p-4 border-b border-white/5 flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-[#18181b] text-white text-xs placeholder-zinc-500 rounded-full pl-10 pr-4 py-2.5 border border-white/10 focus:outline-none focus:border-[#1db954]/60 transition-all"
            />
          </div>
          <button
            type="submit"
            disabled={!query.trim() || isSearching}
            className="bg-[#1db954] text-black font-bold px-4 py-2.5 rounded-full text-xs hover:bg-[#1ed760] disabled:opacity-50 transition-colors cursor-pointer"
          >
          ...
          {isSearching ? (
            ...
            results.map((u) => {
              ...
              return (
                <div
                  key={u._id}
                  className="flex items-center justify-between bg-[#18181b]/60 border border-white/5 p-3 rounded-xl hover:border-white/10 transition-all"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="h-9 w-9 rounded-full bg-zinc-800 text-white font-bold flex items-center justify-center text-xs shrink-0 border border-white/10">
                  ...
                  {status ? (
                    ...
                    <button
                      ...
                      className="bg-zinc-800 text-white hover:bg-[#1db954] hover:text-black border border-white/10 hover:border-transparent text-[11px] font-bold px-3 py-1.5 rounded-full transition-all flex items-center gap-1 shrink-0 cursor-pointer"
                    >
                      {sendingId === u._id ? (
            ...
            <p className="text-center text-xs text-zinc-500 py-8">No users found matching "{query}"</p>
          ) : (
            <p className="text-center text-xs text-zinc-500 py-8">Type a username or email to search for users.</p>
          )}
        </div>
      </div>
    </div>
